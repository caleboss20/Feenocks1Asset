...
function Verification4() {
  ...
  const goback = () => navigate("/verification3");
  const startPopup = () => {
    setIsPopup(true);
    setProgress(0);
    setSecondsLeft(4);
  ...
  useEffect(() => {
    ...
    const step = (timestamp) => {
      ...
      setSecondsLeft(Math.ceil(6 - elapsed / 1000));
      if (elapsed < duration) {
        requestAnimationFrame(step);
      } 
      
      else {
        navigate("/completeverify");
      }
    };
    requestAnimationFrame(step);
  }, [isPopup, navigate]);
    ...
    <div className="relative w-full h-screen p-8 flex flex-col">
      ...
      <div className="max-w-md w-full flex flex-col items-center gap-6">
        <div className="mt-10 flex justify-center flex-col items-center">
          <h2 className="font-medium text-xl mb-10">Scan the Back</h2>
        ...
        {/* Popup with smooth fade */}
        <AnimatePresence>
          {isPopup && (
            <motion.div
              className="backdrop-blur-sm absolute inset-0  flex items-center justify-center bg-black/50 z-50"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            >
              <motion.div
                className="bg-whit rounded-lg p-6 w-full h-100 text-center"
                initial={{ scale: 0.8 }}
                animate={{ scale: 1 }}
                exit={{ scale: 0.8 }}
                transition={{ duration: 0.3 }}
              >
                <h2 className="text-lg font-bold mb-4 text-blue-600">Verifying security features...</h2>
                <div className="rounded-full mt-15 w-full bg-gray-300 h-4 rounded mb-2 overflow-hidden">
                  <motion.div
                ...
                <p className="text-gray-400 font-medium text-lg mt-8">About {secondsLeft} seconds left</p>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
